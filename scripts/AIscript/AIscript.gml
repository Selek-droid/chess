...
if (newY == 0) && (newX - xx == 2) 
{
	...
	if (chosenPiece[0] == KING) 
	{
		if (AIColor == BLACK)  // Kingside, to right
		{
			...
			global.grid[4, 0] = [0, 0];
			global.grid[7, 0] = [0, 0];
			global.grid[6, 0] = [KING, AIColor];
			global.grid[5, 0] = [ROOK, AIColor];
			...
			else if (AIColor == WHITE)  // Queenside, to right
			{
				...
				global.grid[xx, 0] = [0, 0];  // King on xx==3, not 4
				global.grid[7, 0] = [0, 0];
				global.grid[xx + 2, 0] = [KING, AIColor];
				global.grid[xx + 1, 0] = [ROOK, AIColor];
				board_object.NorthCanCastleLeft = false;
				board_object.NorthCanCastleRight = false;
				oGame.turn += 1;
				oGame.state = "Player Turn";
