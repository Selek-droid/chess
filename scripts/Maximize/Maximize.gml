...
var positionScore = 0;
var maxScore = infinity;  // Return value. Eval will compare this to other maxscores from other sent moves.
var listIndex = 0;
var piece = [0 , 0];
...
show_debug_message("MAX scores a final leaf: " + string(positionScore));	
...
{						
	...
	show_debug_message("MAX sends this WHITE move to M I N: (" +  string(oldX) + " , " + string(oldY) + " to (" +
	...
	show_debug_message("MAX receives this score from M I N: (" +  string(minimizedScore));
	if (minimizedScore < maxScore) maxScore = minimizedScore;
	show_debug_message("MAX sets maxscore from M I N: (" +  string(maxScore));
}
searchDepth += 1;
ds_list_destroy(possibleMoves);
return maxScore;